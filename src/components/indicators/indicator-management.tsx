...
export default function IndicatorManagement() {
  ...
  const fetchData = async () => {
    const { data: cats } = await supabase.from('indicator_categories').select('*').order('created_at');
    const { data: inds } = await supabase.from('indicators').select('*').order('number');
    
    if (cats) {
      const merged: IndicatorCategory[] = cats.map(cat => ({
        id: cat.id,
        name: cat.name,
        indicators: (inds || []).filter(ind => ind.category_id === cat.id).map(ind => ({
          id: ind.id,
  ...
  const [editingCategory, setEditingCategory] = useState<IndicatorCategory | null>(null);
  const [deletingCategory, setDeletingCategory] = useState<IndicatorCategory | null>(null);
  ...
  const handleAddIndicator = async (categoryId: string) => {
    ...
    const { data, error } = await supabase
      .from('indicators')
      .insert([{ category_id: categoryId, number: nextNum, text: indicatorText.trim() }])
      .select();
